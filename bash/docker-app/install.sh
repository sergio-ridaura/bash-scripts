...
LAYOUT_CODE() {
  ...
  echo "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.asc] https://download.docker.com/linux/ubuntu $(. /etc/os-release && echo \"$VERSION_CODENAME\") stable" | $IS_ROOT tee /etc/apt/sources.list.d/docker.list >/dev/null
  $IS_ROOT apt-get update
  $IS_ROOT apt-get install $ARG_YES docker-ce docker-ce-cli containerd.io
  if [ "$(whoami)" != "root" ]; then
